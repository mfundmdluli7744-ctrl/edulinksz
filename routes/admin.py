...
def send_async_email(app, msg):
    with app.app_context():
        try:
            mail.send(msg)
            app.logger.info(f"Broadcast email sent successfully to {len(msg.bcc)} recipients.")
        except Exception as e:
            app.logger.error(f"CRITICAL: Failed to send broadcast email. Error: {str(e)}")
            # If it's a connection or auth error, log more details
            import traceback
...
def broadcast_results():
    ...
    subject = "Academic Results Published - EduLinkSZ"
    body = "Dear Parent,\n\nAcademic results for the current term have been published on the EduLinkSZ Portal. Please log in to view your child's report card.\n\nBest regards,\nEduLinkSZ Administration"
    
    recipients = [p.email for p in parents_with_email]
    
    profile = SchoolProfile.query.first()
    sender_name = profile.name if profile and profile.name else "EduLinkSZ"
    sender_email = profile.email if profile and profile.email else current_app.config.get('MAIL_USERNAME')
    
    if not sender_email:
        flash("School email or SMTP username not configured. Please check Settings or Environment Variables.", "danger")
        return redirect(url_for('admin.dashboard'))

    msg = Message(subject, bcc=recipients, sender=(sender_name, sender_email))
    msg.body = body
    
    # Start thread for async sending
    thread = threading.Thread(target=send_async_email, args=(current_app._get_current_object(), msg))
    thread.start()
        
    flash(f"Broadcast message is being sent to {len(parents_with_email)} parents.", "success")
    return redirect(url_for('admin.dashboard'))
